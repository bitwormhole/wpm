...
// Search ...
func (inst *RepositorySearchServiceImpl) Search(ctx context.Context, path fs.Path) ([]*dto.Repository, error) {
...
type myRepositorySearchTask struct {
	...
	maxDepth int
	results  []*dto.Repository
}

func (inst *myRepositorySearchTask) init() {
	inst.maxDepth = 32
	inst.results = make([]*dto.Repository, 0)
}
